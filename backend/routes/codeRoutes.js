...
router.post("/run", protect, async (req, res) => {
  try {
    ...
    console.error("Error executing run route:", error);
    return res.status(500).json({ message: "Execution server error: " + error.message });
  }
});
...
router.post("/submit", protect, async (req, res) => {
  try {
    ...
    console.error("Error executing submit route:", error);
    return res.status(500).json({ message: "Submission server error: " + error.message });
  }
});

// ✅ GET code history for a problem and language
router.get("/history", protect, async (req, res) => {
  try {
    const { problemId, language } = req.query;
    if (!problemId || !language) {
      return res.status(400).json({ message: "problemId and language query parameters are required" });
    }
